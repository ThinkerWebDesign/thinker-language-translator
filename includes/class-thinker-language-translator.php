...
class Thinker_Language_Translator {
	 ...
	 * @return    string     $string     The sanitized output.
	 */
	public function sanitize_style( $sting ) {

		$sting = esc_attr( $sting );
		$sting = trim( $sting );
	 ...
	 * @return    string    $val     Value to filter.
	 */
	 ...
	 * @return    array    $output     Validated settings options.
	 */
		...
		if ( is_array( $input ) ) {
			...
			foreach( $input as $key => $value ) {
				...
				if( isset( $input[ $key ] ) ) {
					...
					if ( in_array( $key, $number_only_values ) ) {
	 ...
	 *
	 * @since     1.0.0
	 * @return    array    $clean_options     Sanitized plugin settings options.
	 */
	 ...
	 * @return    string   $tlt_output   Translator output.
	 */
	public function thinker_translator_output( $tlt_count, $tlt_class ) {
		...
		if ( $tlt_class == 'thinkerLangT-Preview' ) {
		...
		if ( $tlt_class == 'thinkerLangT-Float' ) {
			...
			if ( $options['thinker_translator_bottom'] != '' ) {
				...
				if ( ctype_digit( $tlt_bottom ) && $tlt_bottom != 0 ) {
					$tlt_bottom = $tlt_bottom . 'px';
				}
			} else {
				$tlt_bottom = 'auto';
			}
			if ( $options['thinker_translator_top'] == '' && $options['thinker_translator_bottom'] == '' ) {
				$tlt_top = '50px';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-BottomAuto';
			} else if ( $options['thinker_translator_top'] == 'auto' && $options['thinker_translator_bottom'] == 'auto' ) {
				$tlt_top = '50px';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-BottomAuto';
			} else if ( $options['thinker_translator_top'] == '' && $options['thinker_translator_bottom'] != 'auto' ) {
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-TopAuto';
			} else if ( $options['thinker_translator_top'] == 'auto' ) {
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-TopAuto';
				if ( $options['thinker_translator_bottom'] == '' ) {
					$tlt_bottom = '50px';
				}
			...
			if ( $options['thinker_translator_left'] != '' ) {
				...
				if ( ctype_digit( $tlt_left ) && $tlt_left != 0 ) {
			...
			if ( $options['thinker_translator_right'] != '' ) {
				...
				if ( ctype_digit( $tlt_right ) && $tlt_right != 0 ) {
					$tlt_right = $tlt_right . 'px';
				}
			} else {
				$tlt_right = 'auto';
			}
			if ( $options['thinker_translator_left'] == '0' && $options['thinker_translator_right'] == '0' ) {
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-Center';
			} else if ( $options['thinker_translator_left'] == '' && $options['thinker_translator_right'] == '' ) {
				$tlt_left = '50px';
				$tlt_right = 'auto';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-RightAuto';
			} else if ( $options['thinker_translator_left'] == 'auto' && $options['thinker_translator_right'] == 'auto' ) {
				$tlt_left = '50px';
				$tlt_right = 'auto';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-RightAuto';
			} else if ( $options['thinker_translator_left'] == '' && $options['thinker_translator_right'] != 'auto' ) {
				$tlt_left = 'auto';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-LeftAuto';
			} else if ( $options['thinker_translator_left'] == 'auto' ) {
				$tlt_left = 'auto';
				$tlt_body_class = $tlt_body_class . ' thinkerLangT-LeftAuto';
				if ( $options['thinker_translator_right'] == '' ) {
					$tlt_right = '50px';
			...
			if ( $options['thinker_translator_padding'] != '' ) {
				...
				if ( ctype_digit( $tlt_padding ) && $tlt_padding != 0 ) {
		...
		if ( $options['thinker_translator_hover'] != '' && $options['thinker_translator_inline'] == '' ) {
			...
			if ( $options['thinker_translator_hover_bg'] != '' ) {
				...
				} else if ( $options['thinker_translator_nation'] == 'none' ) {
					$tlt_hover_bg_styles = $tlt_hover_bg_styles . ' background-image: none;';
				} else if ( $options['thinker_translator_nation'] != '' ) {
				...
				if ( $options['thinker_translator_border_radius'] != '' ) {
					...
					if ( ctype_digit( $tlt_border_radius ) && $tlt_border_radius != 0 ) {
		...
		if ( $options['thinker_translator_display'] == 'both' ) {
			$tlt_body_class = $tlt_body_class . ' thinkerLangT-Both';
		} else if ( $options['thinker_translator_display'] == 'text' ) {
		...
		if ( $options['thinker_translator_more'] != '' ) {
			...
			if ( $tlt_count != 1 ) {
				...
				if ( $options['thinker_translator_more'] != 'none' ) {
					...
					';
					if ( $options['thinker_translator_more_wide'] != '' && $options['thinker_translator_inline'] == '' ) {
		...
		if ( $options['thinker_translator_html'] != '' ) {
			...
			if ( $options['thinker_translator_cache'] != '' ) {
				$tlt_url = urlencode( get_home_url() );
			} else {
				if ( is_home() || is_admin() ) {
					$tlt_url = urlencode( get_home_url() );
				} else {
					global $post;
					$tlt_url = urlencode( get_permalink( $post->ID ) );
				}
			}
